...
function Request ( config ) {
  ...
  if (!config.vault[config.ix]) throw new Error('No keys for IX in vault.')
...
Request.prototype.agent = function agent ( returnURL, auth, scopes ) {
  ...
  //   if ( !config.vault[ url.hostname ] ) throw new Error('No keys in vault for "'+host+'"')
  // })
  // ok, make Agent Request
  var credentials = config.vault[config.ix].latest
  var details =
  { header:
    { typ: 'JWS'
    , alg: config.alg
...
Request.prototype.call = function call ( api, params, callback ) {
  ...
  var credentials = config.vault[ apiHost ].latest
  var details =
  { header:
    { typ: 'JWS'
    , alg: config.alg
